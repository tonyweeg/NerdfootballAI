...
| C6 | `survivorCacheUpdater.js` is dead AND pre-broken: calls `onSchedule` at line ~38 without importing it → ReferenceError at module load. Never deployed (index.js is the functions entrypoint and doesn't reference it). Pre-dates this project | Batch A migrated its literals anyway (harmless, keeps guard honest) | **Owner decision:** delete the dead file, or restore it (add the `firebase-functions/v2/scheduler` import and wire its exports). Default if no preference: add the missing import so the file at least loads; deletion stays an owner call |
...
PATTERN="/20[0-9][0-9]/|/20[0-9][0-9]\`|'20[0-9][0-9]'|\"20[0-9][0-9]\""
