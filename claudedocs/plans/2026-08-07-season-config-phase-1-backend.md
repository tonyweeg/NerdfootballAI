...
| # | Site | Facts | Decision |
|---|---|---|---|
| C1 | `espnNerdApi.js:85` (getCurrentWeek, clamp 1-22) and `:338` | ~~Playoff-week detection is intentional divergence~~ **OVERRULED by owner 2026-08-08: "we don't go into playoffs, we stop at week 18."** The 1-22 clamp was legacy accident | ~~Keep local formula~~ **REVISED:** collapse `getCurrentWeek` to `SEASON_CONFIG.utils.getCurrentWeek()` (canonical 18-clamp) like every other site; remove the "playoff clamp intentional" comment; `getWeekDates(week)` keeps its anchor arithmetic (inputs now 1-18). Executed as a C1-revision commit after the initial anchor-only swap (`390fbbb`) |
...
- D3 expected survivors — **CORRECTED against the actual gate run (2026-08-08):** the C2 table files are NOT flagged by this gate (full ISO date strings don't match bare-year patterns — they're covered by the MAIN hardcode guard instead, which does match `20XX-09-0X`; coverage exists, split across the two gates). Actual flagged set: `mlPredictionManager.js` (62, 380, 434 — pending ml_learning per-site item), `realtimeGameSync.js:388` (`users/{uid}/picks/2025/…` — spec-inventory per-site item, pending), `testRTDB.js:46` (throwaway RTDB diagnostic — candidate for archival, owner call), and `index.js:660-691` — **six bare-2025 segments the batch worklists missed**, resolved by D8 below.
- **D8 (gate discovery):** migrate `index.js`'s deletion-function year segments (~660,666,672,678,691) onto their Batch 0 builders (`confidenceUser`/`survivorUser`/`scoresUser`/`weeklyRollupUser`/`survivorEliminations` — built for exactly these shapes). The ~685 line is C4's dead commented-out season-rollup — reword the comment to be non-flaggable without deleting it (owner call still pending).
- **D9 (emulator finding, pre-existing) — ROOT-CAUSED (2026-08-08):** NOT an SDK-namespace issue (firebase-functions 6.4.0's root export IS v2; the calls are valid). The cause is `pickAnalytics.js:653` — `module.exports = { PickAnalyticsEngine }` wholesale-replaces the exports object, discarding all 7 trigger/callable exports assigned above it via the `exports` alias. **Consequence: `onPicksUpdate`, `onLegacyPicksUpdate`, `calculateAnalytics`, `getAnalytics`, `onGameResultUpdate`, `onIndividualGameUpdate` have NEVER deployed** (silently skipped by every deploy for the file's whole history), and Batch D's `onPoolPicksUpdate` inherited the dead file. Additionally: both callables use v1 handler signatures `(data, context)` against v2 `onCall` (would TypeError on every call once exported) and line ~488 has a `functions.https.functions.https.HttpsError` typo.
- **D10 (fix, required before Phase 1 closes):** (1) line 653 → `exports.PickAnalyticsEngine = PickAnalyticsEngine;` (2) convert both callables to v2 signatures (`async (request)` with `request.auth`/`request.data`), preserving response shapes; (3) fix the HttpsError typo; (4) `index.js:~653` deletion loop `week <= 18` → `week <= SEASON_CONFIG.totalWeeks`. Verify by discovery replication (all 7 pickAnalytics exports carry `__endpoint`) and a bounded emulator run showing them initialize.
- **D11 (owner ruling, 2026-08-08):** "the season works like it did last year — no new functions that were left off." All 7 pickAnalytics functions removed from `index.js`'s deploy allowlist (a comment block marks the single re-enable point); `pickAnalytics.js` itself stays healthy per D10. Discovery re-verified: **45 endpoints — deploy surface equivalent to 2025.** This ruling stands as a ground rule for Phases 2-6 (recorded in the spec's Migration Strategy).
- **Recorded findings (not Phase 1 blockers):** F1 — the never-deployed analytics subsystem is a significant pre-existing product fact for the owner (any analytics believed maintained by these triggers was never written). F2 — ZERO test coverage exercises any Cloud Function (two deliberate mutations of the new shared trigger core passed 29/29); the Phase 6 emulator flip drill is the first functional exercise — consider a minimal functions test harness there. F3 — the games rules' `request.auth == null` write-grant is exposure-neutral as copied but its "Allow Firebase Functions" justification is factually wrong (Admin SDK bypasses rules); flag for a security pass. F4 — `deepStar6User`'s broad catches swallow the new loud guard throws (pre-existing catch style); consider narrowing to not-found in a later pass.
