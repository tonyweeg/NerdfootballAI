...
- **D9 (emulator finding, pre-existing):** none of `pickAnalytics.js`'s 7 functions (including the new `onPoolPicksUpdate` and pre-existing `onLegacyPicksUpdate`) appear in emulator function discovery — the file uses `require('firebase-functions')` root namespace with v2-style `onDocumentWritten`, unlike registering files that import `firebase-functions/v2/firestore`. Root-cause and disposition assigned to the Batch D quality review: determine definitively whether these triggers deploy in production shape, and whether `onPoolPicksUpdate` (the Phase 1 deliverable) is actually live-able as written.
