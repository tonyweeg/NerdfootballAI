...
3. `public/game-data/nfl_{year}_week_{n}.json` for weeks 1-18 — the EXISTING consumed format: an **ID-keyed map** `{"101": {a, h, dt, stadium}, ...}` (the plan's original `{week, games:[...]}` description was wrong; the implementer verified the real consumer — confidence page's weekBible — and the live files). Id numbering `BASE_GAME_ID(100) + week*100 + index + 1`.
   ...
   **`dt` convention (CORRECTED after spec review, 2026-08-08 — decisive):** game-data `dt` uses the **legacy bare-Z-meaning-Eastern convention** (`"2026-09-10T20:20:00Z"` where the digits are Eastern wall-clock) — NOT explicit-offset ISO. Reason, empirically proven: `easternTimeParser-v2.js` (which drives pick-locking and game-time display on the confidence and survivor pages) unconditionally strips `Z` and subtracts a fixed 4 hours; feeding it offset-ISO shifts computed game times by 3-4h for every non-Eastern viewer. Ground rule: 2026 files must behave byte-for-byte like 2025's did. The explicit-offset convention remains correct for `season-data.js`'s `kickoffDateTime`/`seasonEndDate` (different consumer path, verified safe). Two pre-existing quirks preserved deliberately, NOT fixed: (a) the parser's fixed 4h correction is 1h off for January (EST) games — 2025 ran that way; (b) `nerdfootballTheGrid.html`'s `getGamesForWeek` expects a `.games` array and already returns `[]` against real files.
   Required unit test: replicate the parser's strip-and-subtract on a generated September `dt` and assert it yields the game's true UTC instant.
